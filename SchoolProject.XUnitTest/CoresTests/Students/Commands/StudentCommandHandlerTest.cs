...
using Moq;
using SchoolProject.Core.Features.Students.Commands.Haundlers;
...
namespace SchoolProject.XUnitTest.CoresTests.Students.Commands
{
	public class StudentCommandHandlerTest
	{
		...
		public async Task Handle_AddStudent_Should_Add_Data_And_StatusCode201()
		{
			...
			var addStudentCommand = new AddStudentCommand() { NameAr = "محمد", Name = "Mohamed" };
			_studentServiceMock.Setup(x => x.AddStudent(It.IsAny<Student>())).Returns(Task.FromResult("Success"));
			//act
			var result = await handler.Handle(addStudentCommand, default);
			//Assert
			result.Succeeded.Should().BeTrue();
			result.StatusCode.Should().Be(HttpStatusCode.Created);
			_studentServiceMock.Verify(x => x.AddStudent(It.IsAny<Student>()), Times.Once, "Not Called");
		}
		[Fact]
		public async Task Handle_AddStudent_Should_return_StatusCode400()
		{
			...
			var handler = new StudentCommandHandler(_studentServiceMock.Object, _mapperMock, _localizerMock.Object);
			var addStudentCommand = new AddStudentCommand() { NameAr = "محمد", Name = "Mohamed" };
			_studentServiceMock.Setup(x => x.AddStudent(It.IsAny<Student>())).Returns(Task.FromResult(""));
			//act
			var result = await handler.Handle(addStudentCommand, default);
			//Assert
		...
		public async Task Handle_UpdateStudent_Should_Return_StatusCode404()
		{
			//Arrange
			var handler = new StudentCommandHandler(_studentServiceMock.Object, _mapperMock, _localizerMock.Object);
			var updateStudentCommand = new EditStudentCommand() { Id = 6, NameAr = "محمد", Name = "Mohamed" };
			Student? student = null;
			int xResult = 0;
			_studentServiceMock.Setup(x => x.GetStudentByIdAsync(updateStudentCommand.Id)).Returns(Task.FromResult(student)).Callback((int x) => xResult = x);
			//act
			...
			_studentServiceMock.Verify(x => x.GetStudentByIdAsync(It.IsAny<int>()), Times.Once, "Not Called");
		}
	}
}
